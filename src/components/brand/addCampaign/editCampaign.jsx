...
export default function EditCampaignWizard({ setShowModal, campaignId }) {
  ...
  const [campaign, setCampaign] = useState({
    ...
    productName: "",
    productDescription: "",
    brandLogo: null,           // can be either a File or a URL‐string
    productImages: [],         // array of File or URL‐string
    ...
    toneMessage: "",           // optional
    referenceContent: "",      // optional URL
    referenceFile: null,       // optional File
...
console.log("[EditCampaignWizard] selected object:", selected);
  ...
  setCampaign({
    ...
    productImages: Array.isArray(selected.productImages) ? selected.productImages : [],
    numCreators: selected.numCreators != null ? String(selected.numCreators) : "",

    // Step 2: Content Guidelines
    contentPlatform: selected.contentPlatform || "",
    requiredHashtags: Array.isArray(selected.requiredHashtags) ? selected.requiredHashtags : [],
    ...
    categoryDetails: selected.categoryDetails
      ...
      : {
          Beauty: { productType: "", skinTypes: [], keyIngredients: "", howToUse: "" },
          Food: { preparationMethod: "", dietaryTags: [], eatingScene: "" },
          Beverage: { servingType: "", servingTemperature: "", caffeineContent: "", dietaryTags: [] },
          "Wellness & Supplements": { productType: "", targetFunction: [], formType: "", usageTiming: "", flavor: "", dietaryTags: [] },
          "Personal Care": { productType: "", skinBodyAreas: [], keyIngredients: "", scentFlavor: "", textureForm: "" }
        },
    ...
    recruitmentEndDate: selected.recruitmentEndDate ? selected.recruitmentEndDate.slice(0, 10) : "",
  ...
  setError(null);
     ...
     // dobara fetch selected, for safety
   const selected = Campaigns.find(c => c._id === campaignId);
   if (!selected) {
  ...
  try {
    // 1) Upload brandLogo if it's a new File
    let brandLogoUrl = campaign.brandLogo;
    if (campaign.brandLogo instanceof File) {
      brandLogoUrl = await uploadFile(campaign.brandLogo, 'brandLogo');
    }

    // 2) Upload productImages (preserve existing URLs)
    const productImageUrls = await Promise.all(
      (campaign.productImages || []).map(async (img) => {
        if (typeof img === 'string') return img;
        return await uploadFile(img, 'productImages');
      })
    );

    // 3) Upload referenceFile if new, else use existing URL or null
    let referenceFileUrl = selected.referenceFile || null;
    if (campaign.referenceFile instanceof File) {
      referenceFileUrl = await uploadFile(campaign.referenceFile, 'referenceFile');
    }

    // 4) Build payload matching backend schema
    const payload = {
      ...
      numCreators: Number(campaign.numCreators),
      applicantsToShow: CREATOR_OPTIONS.find(
        (opt) => opt.value === Number(campaign.numCreators)
      )?.applicantsToShow,
      contentPlatform: campaign.contentPlatform,
      requiredHashtags: campaign.requiredHashtags,
      mentionHandle: campaign.mentionHandle.trim(),
      toneMessage: campaign.toneMessage.trim() || null,
      referenceContent: campaign.referenceContent.trim() || null,
      referenceFile: referenceFileUrl,
      categoryDetails: campaign.categoryDetails[campaign.category] || {},
      influencersReceive: campaign.influencersReceive.trim() || null,
      participationRequirements: campaign.participationRequirements.trim() || null,
      deadline: campaign.deadline || null,
      recruitmentEndDate: campaign.recruitmentEndDate || null,
      status: campaign.status,
    };

    // 5) Send PUT request to update
    const selected = Campaigns.find(c => c._id === campaignId);
    const idx = Campaigns.findIndex(c => c._id === campaignId);
    const token = localStorage.getItem("BRAND_TOKEN");
    const resp = await axios.put(
      `${config.BACKEND_URL}/brand/campaign-request/request/${selected._id}`,
      { campaign: payload },
    ...
    if (resp.data.status === 'success') {
      // 6) Sync local state
      await EditCampaign(idx, { ...selected, ...payload });
      ...
      setError(resp.data.message || 'Failed to update campaign');
    }
  } catch (err) {
    console.error('Error in EditCampaignWizard:', err);
                  ...
                  <div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        toast.info("Load from Previous Campaign not implemented", {
                          theme: "dark",
                        });
                      }}
                        ...
                        {[
                          ...
                          >
                            <input
                              ...
                              onChange={() => {
                                const already =
                                  campaign.categoryDetails.Beauty.skinTypes.includes(stype);
                                const updated = already
                                  ? campaign.categoryDetails.Beauty.skinTypes.filter(
                                      (x) => x !== stype
                                    )
                                  : [...campaign.categoryDetails.Beauty.skinTypes, stype];
                                handleCategoryDetailChange(
                                  "Beauty",
                                  "skinTypes",
                                  updated
                ...
                {campaign.category === "Food" && (
                  <div className="space-y-4">
                    ...
                    <InputField
                      ...
                      onChange={(e) =>
                        handleCategoryDetailChange("Food", "eatingScene", e.target.value)
                      }
                ...
                {campaign.category === "Beverage" && (
                  <div className="space-y-4">
                    <InputField
                      ...
                      onChange={(e) =>
                        handleCategoryDetailChange("Beverage", "servingType", e.target.value)
                      }
                        ...
                        {["Contains Caffeine", "Caffeine-Free", "Low Caffeine"].map((opt) => (
                          ...
                          >
                            <input
                              ...
                              value={opt}
                              checked={
                                campaign.categoryDetails.Beverage.caffeineContent === opt
                              }
                              onChange={() =>
                ...
                {campaign.category === "Wellness & Supplements" && (
                  <div className="space-y-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[
                          ...
                          >
                            <input
                              type="checkbox"
                              checked={campaign.categoryDetails["Wellness & Supplements"].targetFunction.includes(
                                f
                              )}
                              onChange={() => {
                                const arr =
                                  campaign.categoryDetails["Wellness & Supplements"].targetFunction;
                                const already = arr.includes(f);
                                const updated = already ? arr.filter((x) => x !== f) : [...arr, f];
                                handleCategoryDetailChange(
                                  "Wellness & Supplements",
                                  "targetFunction",
                                  updated
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {["Vegan", "Gluten-Free", "Sugar-Free", "Other"].map((tag) => (
                          <label
                            key={tag}
                            className="flex items-center space-x-2 cursor-pointer hover:bg-gray-800 p-2 rounded-md"
                          >
                            <input
                              type="checkbox"
                              checked={
                                campaign.categoryDetails["Wellness & Supplements"].dietaryTags.includes(
                                  tag
                                )
                              }
                              onChange={() => {
                                const arr =
                                  campaign.categoryDetails["Wellness & Supplements"].dietaryTags;
                                const already = arr.includes(tag);
                                const updated = already ? arr.filter((x) => x !== tag) : [...arr, tag];
                                handleCategoryDetailChange(
                                  "Wellness & Supplements",
                                  "dietaryTags",
                                  updated
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[
                          ...
                          >
                            <input
                              type="checkbox"
                              checked={campaign.categoryDetails["Personal Care"].skinBodyAreas.includes(
                                area
                              )}
                              onChange={() => {
                                const arr =
                                ...
                                const updated = already ? arr.filter((x) => x !== area) : [...arr, area];
                                handleCategoryDetailChange(
                                  "Personal Care",
                                  "skinBodyAreas",
                                  updated
                  ...
                  <div className="bg-gray-800 p-4 rounded-lg">
                    ...
                    {campaign.category === "Wellness & Supplements" && (
                      <div className="space-y-1">
                        ...
                        {campaign.categoryDetails["Wellness & Supplements"].targetFunction.length >
                          0 && (
                          <p>
                        ...
                        {campaign.categoryDetails["Wellness & Supplements"].dietaryTags.length >
                          0 && (
                          <p>
                            <span className="font-semibold">Dietary Tags:</span>{" "}
